		...
		case 1 : 
			...
			if (_Imlager >= 300) then 
			...
			else
				{
					...
					while {_i > 0} do 
						{
							_res = _resbox createVehicle ["CargoNet_01_box_F", _Ort , [], 4, "NONE"];
							_i = _i-1;
						};
				};
			ResVert set [0,_Imlager];
		...
		case 4 : 
			{
				...
				if (_Imlager >= 1500) then 
				{
					...
					Lager_Lamentin addAction ["<t color = '#77C3E0'> Upgrade storage to stage 3 [4000 Res]","Base\Lager.sqf",[5,Hilfe_Lamentin]];
					_Lager removeAction _idup;
				}
		...
		case 10:
			{
				...
				_Kiste addAction ["<t color = '#ED0909'> DESTROY INVENTORY AND BOX",{_Kiste = _this select 0; deleteVehicle _Kiste}];
				_Kiste addAction ["<t color = '#EDCF24'> Pick up box","Base\kistepickup.sqf"];
			}
		};
