...
_kleineaudohs = ["O_Quadbike_01_F","C_Quadbike_01_F"];
...
_faktor = 1;
_vehart = "";
if (typeOf _veh in _Panzer) then {_vehart = "Panzer";};
if (typeOf _veh in _MRAP) then {_vehart = "MRAP";};
if (typeOf _veh in _LKW) then {_vehart = "LKW";};
if (typeOf _veh in _audohs) then {_vehart = "audohs";};
if (typeOf _veh in _kleineaudohs) then {_vehart = "kleineaudohs";};
if (typeOf _veh in _Hubschrauber) then {_vehart = "Hubschrauber";};
if (typeOf _veh in _kleineHubschrauber) then {_vehart = "kleineHubschrauber";};

switch (_vehart) do 
	{
	case ("Panzer") : {_faktor = 1100;};
	case ("MRAP") : {_faktor = 500;};
	case ("LKW") : {_faktor = 350;};
	case ("audohs") : {_faktor = 200;};
	case ("kleineaudohs") : {_faktor = 50;};
	case ("Hubschrauber") : {_faktor = 900;};
	case ("kleineHubschrauber") : {_faktor = 700;};
	};
if((typeOf _veh) in _Fahrzeuge) then 			//wenn Spieler ein Fahrzeug ist
	{
	_speed = speed _veh;						//Geschw. abholen
	_schaden = damage _veh;
	_treibstoff = fuel _veh;
	_ImLager = ResVert select 0;
	if (_speed <= 3) then //wenn Spieler zumindest fast steht
		{
		_BenRes = _schaden * _faktor + (1 - _treibstoff) * (_faktor * 0.2);
		if (_BenRes <= _ImLager) then {
			...
			sleep 20;
			_veh setDamage 0;
			_Versorgpunkt addAction ["<t color = '#4EB555'> Repair and refuel","Base\Versorgpunkt\reparieren.sqf"];
			_veh setFuel 1;
			hint "Vehicle ready!";
	...
	}else {hint "You cant heal yourself with a screwdriver!"};