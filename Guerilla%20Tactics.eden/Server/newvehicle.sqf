_veh = _this select 0;
	if ((typeOf _veh) in UPG_2) then
	{
		...
		if ((typeOf _veh) in UPG_AMMO) then 
			...
			if ((typeOf _veh) == "C_Plane_Civil_01_F") then {
				[_veh,["Airshow","vehicles\flugzeug\rauch.sqf"]] remoteExec ["addAction",0,true];
				[_veh,["Load Box","vehicles\flugzeug\flugzeugfracht.sqf"]] remoteExec ["addAction",0,true];
				[_veh,["Attach Barrelbombs","vehicles\flugzeug\Fassbombe.sqf"]] remoteExec ["addAction",0,true];
				}
			};
		};
publicVariable "Fahr_geupgr";