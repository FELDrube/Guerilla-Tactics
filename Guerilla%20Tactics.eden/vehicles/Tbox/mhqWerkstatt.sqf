...
MHQ1name = {_obj = _this select 0; [_obj,"MHQ1"] remoteExec ["setVehicleVarName",0,true]; MHQ1alive = true; publicVariable "MHQ1alive";};
MHQ2name = {_obj = _this select 0; [_obj,"MHQ2"] remoteExec ["setVehicleVarName",0,true]; MHQ2alive = true; publicVariable "MHQ1alive";};
MHQ3name = {_obj = _this select 0; [_obj,"MHQ3"] remoteExec ["setVehicleVarName",0,true]; MHQ3alive = true; publicVariable "MHQ1alive";};
MHQantiname = {_obj = _this select 0;[_obj,""] remoteExec ["setVehicleVarName",0,true];};
if ([Fahr_geupgr, _truckpathlong] call BIS_fnc_returnNestedElement) exitWith {		// wenn das MHQ element schon auf true steht
...
if ((count _list) != 1) exitWith {hint "No Workshop around!"; [_truck, _id] remoteExec ["removeAction",0,true];};
if (_ImLager < 900) exitWith {hint "Not enough Resources!";};
if ((MHQ1alive)&&(MHQ2alive)&&(MHQ3alive)) then {hint "There are too many MHQs active at the moment"   //wenn es 3 MHQs gibt dann mach nix und zeig den text an
	...
	_truck addMPEventHandler ["MPKilled",
	{
		...
		switch (_Name) do {
			case "MHQ1" : {MHQ1alive = false; ResVert set [1,0]; publicVariable "MHQ1alive";};
			case "MHQ2" : {MHQ2alive = false; ResVert set [2,0]; publicVariable "MHQ2alive";};
			case "MHQ3" : {MHQ3alive = false; ResVert set [3,0]; publicVariable "MHQ3alive";};
		};
	...
	if (!MHQ1alive) then {_param remoteExec ["MHQ1name"];} else {
		if (!MHQ2alive) then {_param remoteExec ["MHQ2name"];} else {
			if (!MHQ3alive) then {_param remoteExec ["MHQ3name"];};};};				// wenn MHQcount = x, nenne den LKW für alle Spieler MHQ x"      	
	...
	_Resneu = (ResVert select 0) - 900;
	ResVert set [0,_Resneu];
	publicVariable "ResVert";
	

	[_truck, ["<t color ='#EDCF24'>Scan for resources",{				// aktion für resourcen-scan einfügen
	//----------------------------------------------------------------------------------------------------------------------------------------
		_MHQ = _this select 0;
	...
	//-------------------------------------------------------------------------------------------------------------------------------------
	[_truck, ["<t color ='#EDCF24'>Unload resources",{
		...
		while {_i > 0} do {															
			_res = createVehicle ["CargoNet_01_box_F", getPos _MHQ, [], 7, "NONE"];				// i kisten spawnen
			_i = _i -1;	
			};
			switch (_Name) do {
